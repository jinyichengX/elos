...
#define LIST_HEAD_CREAT(list_head_name) struct list_head list_head_name
...
static  void list_add_inorder(struct list_head *new, struct list_head *head)
{
...
/* 从列表中寻找对应的元素，成功则返回true */
static int list_find(struct list_head *list,
				struct list_head *head)
{
	struct list_head *pos,*tmp;
	list_for_each_safe(pos, tmp, head)
	{
		if(pos == list) return 1;
	}
	return 0;
}
